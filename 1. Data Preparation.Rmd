...
clean <- data %>%
  ...
  fill(Year, Authors, Title, Country, .direction = "down") %>% # Fill down empty cells
  clean_names() %>%                       # Rename column names
  rename(mp_type = type_of_mp,            
...
Lets also standardize country names to make it easier to join new data later
...
Theory is that there should be more market power in the industries which cannot 
store product for a long time.

```{r}
perishables <- c("beef", "pork", "meat", "milk", "paultry", 
                 "poultry", "egg", "fish", "salmon", "hog", "dairy", "cheese")
...
clean %>%
  ggplot(aes(x = as_factor(perish),
             ...
             color = approach)) + 
  geom_jitter(width = 0.25, height = 0, alpha = 0.6, size = 2) +
  labs(x = "Perishable goods", y="Market Power Index") +
  theme_minimal()
```
Surprisingly, perishable goods seems to experience less MP


## Outliers
Finally, lets take a look on the possible MP outliers
...
```{r}
clean <- clean %>%
  filter(between(mp_index, 0.001, 0.52))
```

Looks like I am done with data preparation of the initial MP data 
...
# Joining data from external sorces 
...
```{r}
sum(is.na(mean_pse$mean_pse))
```
Unfortunately 10 or (4.5%) of observations is missing
...
Probably I should not observations with pp > 2 in further analysis 
...
To decrease the number of dropped observations we assume that there is NO
