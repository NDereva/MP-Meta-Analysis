...
## The same data but categorised by industry
...
We need to transform industry to match the main dataset
```{r}
farms_full %>% 
  distinct(industry) 
```
...
farms_full %>% 
  select(industry) %>% 
  distinct() %>%
  mutate(industry_cat = map_chr(industry, ~ classify_industry(.x))) 
...
farms_full_to_join <- farms_full %>%
  ...
  mutate(country = map_chr(country, ~ standatise_country_name(.x)))
